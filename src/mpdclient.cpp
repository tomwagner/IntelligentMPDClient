...
MPD::Client::Client() : itsConnection(0),
...
bool MPD::Client::Connected() const {
  return itsConnection;
}


void MPD::Client::Disconnect() {
  if (itsConnection)
    mpd_connection_free(itsConnection);
  if (itsStats)
    mpd_stats_free(itsStats);
...
unsigned MPD::Client::Version() const {
  return itsConnection ? mpd_connection_get_server_version(itsConnection)[1] : 0;
}


void MPD::Client::SetHostname(const std::string &host) {
  size_t at = host.find("@");
...
void MPD::Client::SetStatusUpdater(StatusUpdater updater, void *data) {
  itsUpdater = updater;
  itsStatusUpdaterUserdata = data;
}


void MPD::Client::SetErrorHandler(ErrorHandler handler, void *data) {
  itsErrorHandler = handler;
  itsErrorHandlerUserdata = data;
}


void MPD::Client::GoIdle() {
  if (supportsIdle && !itsIdleBlocked && !isIdle && mpd_send_idle(itsConnection))
    isIdle = 1;
}


int MPD::Client::GoBusy() {
  int flags = 0;
...
bool MPD::Client::UpdateDirectory(const std::string &path) {
  if (!itsConnection)
    return false;
...
void MPD::Client::Pause(bool state) {
  if (!itsConnection)
    return;
  if (!isCommandsListEnabled) {
    GoBusy();
...
void MPD::Client::Move(unsigned from, unsigned to) {
  if (!itsConnection)
    return;
  if (!isCommandsListEnabled) {
    GoBusy();
...
void MPD::Client::Swap(unsigned from, unsigned to) {
  if (!itsConnection)
    return;
  if (!isCommandsListEnabled) {
    GoBusy();
...
void MPD::Client::Seek(unsigned where) {
  if (!itsConnection)
    return;
  if (!isCommandsListEnabled) {
    GoBusy();
...
void MPD::Client::ClearPlaylist(const std::string &playlist) {
  if (!itsConnection)
    return;
  if (!isCommandsListEnabled) {
    GoBusy();
...
void MPD::Client::AddToPlaylist(const std::string &path, const Song &s) {
  if (!s.Empty())
    AddToPlaylist(path, s.GetFile());
}


void MPD::Client::AddToPlaylist(const std::string &path, const std::string &file) {
  if (!itsConnection)
    return;
  if (!isCommandsListEnabled) {
    GoBusy();
...
void MPD::Client::Move(const std::string &path, int from, int to) {
  if (!itsConnection)
    return;
  if (!isCommandsListEnabled)
    GoBusy();
...
bool MPD::Client::Rename(const std::string &from, const std::string &to) {
  if (!itsConnection)
    return false;
  if (!isCommandsListEnabled) {
    GoBusy();
...
void MPD::Client::GetPlaylistChanges(unsigned version, SongList &v) {
  if (!itsConnection)
    return;
  assert(!isCommandsListEnabled);
  if (!version)
...
MPD::Song MPD::Client::GetSong(const std::string &path) {
  if (!itsConnection)
    return Song();
  assert(!isCommandsListEnabled);
  GoBusy();
...
void MPD::Client::GetPlaylistContent(const std::string &path, SongList &v) {
  if (!itsConnection)
    return;
  assert(!isCommandsListEnabled);
  GoBusy();
...
void MPD::Client::GetSupportedExtensions(std::set<std::string> &acc) {
  if (!itsConnection)
    return;
  assert(!isCommandsListEnabled);
  GoBusy();
...
void MPD::Client::SetRepeat(bool mode) {
  if (!itsConnection)
    return;
  if (!isCommandsListEnabled) {
    GoBusy();
...
void MPD::Client::SetRandom(bool mode) {
  if (!itsConnection)
    return;
  if (!isCommandsListEnabled) {
    GoBusy();
...
void MPD::Client::SetSingle(bool mode) {
  if (!itsConnection)
    return;
  if (!isCommandsListEnabled) {
    GoBusy();
...
void MPD::Client::SetConsume(bool mode) {
  if (!itsConnection)
    return;
  if (!isCommandsListEnabled) {
    GoBusy();
...
void MPD::Client::SetVolume(unsigned vol) {
...
std::string MPD::Client::GetReplayGainMode() {
  if (!itsConnection)
    return "Unknown";
  assert(!isCommandsListEnabled);
  GoBusy();
...
void MPD::Client::SetReplayGainMode(ReplayGainMode mode) {
  if (!itsConnection)
    return;
  const char *rg_mode;
  switch (mode) {
...
//    default:
      ...
      //TODO FatalError("undefined value of ReplayGainMode!");
  }
  if (!isCommandsListEnabled)
    GoBusy();
  else
...
void MPD::Client::SetCrossfade(unsigned crossfade) {
  if (!itsConnection)
    return;
  if (!isCommandsListEnabled) {
    GoBusy();
...
int MPD::Client::AddSong(const std::string &path, int pos) {
  if (!itsConnection)
    return -1;
  int id = -1;
...
int MPD::Client::AddSong(const Song &s, int pos) {
  //TODO return !s.Empty() ? (AddSong((!s.isFromDB() ? "file://" : "") + (s.Localized() ? locale_to_utf_cpy(s.GetFile()) : s.GetFile()), pos)) : -1;
  return !s.Empty() ? (AddSong((!s.isFromDB() ? "file://" : "") + s.GetFile(), pos)) : -1;
}


void MPD::Client::Add(const std::string &path) {
  if (!itsConnection)
    return;
  if (!isCommandsListEnabled) {
    GoBusy();
...
bool MPD::Client::AddRandomTag(mpd_tag_type tag, size_t number) {
...
bool MPD::Client::AddRandomSongs(size_t number) {
  if (!itsConnection && !number)
    return false;
  assert(!isCommandsListEnabled);
...
bool MPD::Client::Delete(unsigned pos) {
  if (!itsConnection)
    return false;
  if (!isCommandsListEnabled)
    GoBusy();
...
bool MPD::Client::DeleteID(unsigned id) {
  if (!itsConnection)
    return false;
  if (!isCommandsListEnabled)
    GoBusy();
...
bool MPD::Client::Delete(const std::string &playlist, unsigned pos) {
  if (!itsConnection)
    return false;
  if (!isCommandsListEnabled) {
    GoBusy();
...
void MPD::Client::StartCommandsList() {
  if (!itsConnection)
    return;
  assert(!isCommandsListEnabled);
  GoBusy();
...
bool MPD::Client::CommitCommandsList() {
  if (!itsConnection)
    return false;
  assert(isCommandsListEnabled);
  assert(!isIdle);
...
bool MPD::Client::DeletePlaylist(const std::string &name) {
  if (!itsConnection)
    return false;
  if (!isCommandsListEnabled) {
    GoBusy();
...
bool MPD::Client::LoadPlaylist(const std::string &name) {
  if (!itsConnection)
    return false;
  assert(!isCommandsListEnabled);
  GoBusy();
...
int MPD::Client::SavePlaylist(const std::string &name) {
  if (!itsConnection)
    return false;
  assert(!isCommandsListEnabled);
  GoBusy();
...
void MPD::Client::GetPlaylists(TagList &v) {
  if (!itsConnection)
    return;
  ItemList list;
  GetDirectory("/", list);
...
void MPD::Client::GetList(TagList &v, mpd_tag_type type) {
  if (!itsConnection)
    return;
  assert(!isCommandsListEnabled);
  GoBusy();
...
void MPD::Client::StartSearch(bool exact_match) {
  if (itsConnection)
    mpd_search_db_songs(itsConnection, exact_match);
}


void MPD::Client::StartFieldSearch(mpd_tag_type item) {
  if (itsConnection) {
...
void MPD::Client::AddSearch(mpd_tag_type item, const std::string &str) const {
...
void MPD::Client::AddSearchAny(const std::string &str) const {
  assert(!str.empty());
  if (itsConnection)
...
void MPD::Client::AddSearchURI(const std::string &str) const {
  assert(!str.empty());
  if (itsConnection)
...
void MPD::Client::CommitSearch(SongList &v) {
  if (!itsConnection)
    return;
  assert(!isCommandsListEnabled);
  GoBusy();
...
void MPD::Client::CommitSearch(TagList &v) {
  if (!itsConnection)
    return;
  assert(!isCommandsListEnabled);
  GoBusy();
...
void MPD::Client::GetDirectory(const std::string &path, ItemList &v) {
  if (!itsConnection)
    return;
  assert(!isCommandsListEnabled);
  GoBusy();
...
void MPD::Client::GetDirectoryRecursive(const std::string &path, SongList &v) {
  if (!itsConnection)
    return;
  assert(!isCommandsListEnabled);
  GoBusy();
...
void MPD::Client::GetSongs(const std::string &path, SongList &v) {
  if (!itsConnection)
    return;
  assert(!isCommandsListEnabled);
  GoBusy();
...
void MPD::Client::GetDirectories(const std::string &path, TagList &v) {
  if (!itsConnection)
    return;
  assert(!isCommandsListEnabled);
  GoBusy();
...
void MPD::Client::GetOutputs(OutputList &v) {
  if (!itsConnection)
    return;
  assert(!isCommandsListEnabled);
  GoBusy();
...
bool MPD::Client::EnableOutput(int id) {
  if (!itsConnection)
    return false;
  if (!isCommandsListEnabled) {
    GoBusy();
...
bool MPD::Client::DisableOutput(int id) {
  if (!itsConnection)
    return false;
  if (!isCommandsListEnabled) {
    GoBusy();
...
void MPD::Client::GetURLHandlers(TagList &v) {
  if (!itsConnection)
    return;
  assert(!isCommandsListEnabled);
  GoBusy();
...
void MPD::Client::GetTagTypes(TagList &v) {
  if (!itsConnection)
    return;
  assert(!isCommandsListEnabled);
  GoBusy();
