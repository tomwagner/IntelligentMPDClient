...
void MPD::Song::SetTrack(unsigned track, unsigned pos) {
  SetTag(MPD_TAG_TRACK, pos, utils::intToString(track));
}
...
void MPD::Song::SetDate(unsigned year, unsigned pos) {
  SetTag(MPD_TAG_DATE, pos, utils::intToString(year));
}
...
bool MPD::Song::isFormatOk(const std::string &type, const std::string &s) {
  ...
  for (size_t i = s.find('%'); i != std::string::npos; i = s.find('%', i)) {
    ...
    if (!toGetFunction(s[i])) {
      std::cerr << type << ": invalid character at position " << utils::intToString(i + 1) << ": '" << s[i] << "'\n";
      return false;
    }
  }
  return true;
...
MPD::Song::GetFunction MPD::Song::toGetFunction(char c) {
  switch (c) {
    case 'l':
