...
namespace GUI {
  ...
  it(NULL) {
    ...
    builder->get_widget("playImageSmall", playImageSmall);
    builder->get_widget("pauseImageSmall", pauseImageSmall);
  ...
  void SlideshowWidget::setSlideshowWidget(std::map<std::string, SSlide *>* list) {
    slideList = list;
    it = slideList->begin();
  ...
  void SlideshowWidget::on_slideshowStart() {
    if (slideSlideshow->get_active()) {
      run();
      slideSlideshow->set_image(*pauseImageSmall);
    } else {
      stop();
      slideSlideshow->set_image(*playImageSmall);
    }
  ...
  void SlideshowWidget::run() {
    ...
    if (!slideshowStarted) {
      ...
      updateTimeout = Glib::signal_timeout().connect(
              sigc::mem_fun(*this, &SlideshowWidget::slideLeft), 5000);
    }
  }
